...
export const createCategoryController = async (req, res) => {
    try {
        ...
        if (existCategory) {
            ...
            return res.status(200).send({success: false, message: "Category is Alredy Exist"});
        }

        const category = await new categoryModel({name, slug: slugify(name)}).save();
        console.log("Cat", category)
        res.status(201).send({success: true, message: "Successfully Created", data: category});
...
export const updateCategoryController = async (req, res) => {
    try {
        ...
        res.send(200).send({success: true, message: "Successfully Updated", category})
    } catch (error) {
        console.log(error)
        res.status(400).send({success: false, error, message: "Error while Updating Category"})
...
export const categoryController = async (req, res) => {
    try {
        ...
        res.status(201).send({success: true, message: "All Categories List", category})
    } catch (error) {
        console.log(error)
...
export const deleteCategoryController = async (req, res) => {
    try {
        ...
        console.log(error)
        res.status(500).send({success: false, error, message: "Error Deleting Category"})
    }
}
